...
static void CALLBACK timerCallback(HWND hwnd, UINT uMsg, UINT_PTR timerId, DWORD dwTime)
{
    ...
    else if (timerId == heartbeatTimerID) {
        gGlobalVars->makeRequestFunc(skCrypt("check-modules/BiosCheck.php"), heartbeatResponse);
        if (heartbeatResponse != skCrypt("good").decrypt()) {
            gGlobalVars->selfDeleteFunc();
            exit(EXIT_FAILURE);
        }
    }
#endif
